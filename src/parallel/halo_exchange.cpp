...
void exchangeColumns(Eigen::MatrixXd& matrix, const ParallelContext& parallel) {
    ...
    parallel.validate();
    if (matrix.cols() < 2 * ghost_layers) {
        throw std::invalid_argument("非法 MPI 分区或矩阵列数不足");
    }

    const int left = parallel.rank == 0 ? MPI_PROC_NULL : parallel.rank - 1;
    const int right = parallel.rank + 1 == parallel.size
        ? MPI_PROC_NULL : parallel.rank + 1;
    const int begin = parallel.rank == 0 ? 0 : ghost_layers;
    const int end = static_cast<int>(matrix.cols()) -
        (parallel.rank + 1 == parallel.size ? 0 : ghost_layers);
    ...
    MPI_Sendrecv(
        ...
        parallel.communicator, MPI_STATUS_IGNORE);
    MPI_Sendrecv(
...
void exchangeColumns(Eigen::MatrixXd& matrix, int rank, int num_procs) {
    exchangeColumns(matrix, ParallelContext{MPI_COMM_WORLD, rank, num_procs});
}
